...
Streamlit Community Cloud currently defaults to Python 3.14, and
``tensorflow-cpu`` has no cp314 wheels. The saved ``.weights.h5`` files are
Keras-native checkpoints (embedding + 2× BiLSTM + dense), so we run the same
forward pass in NumPy and keep TensorFlow out of the deploy path.
"""
